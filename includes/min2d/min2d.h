...
namespace Min2D {
	...
	inline void Clear(const Color &clr) {
		glClearColor(clr.red / 255.0f, clr.green / 255.0f, clr.blue / 255.0f, clr.alpha / 255.0f);
	...
	inline Color GetPixel(const Point &pt) {
		Color clr;
		glReadPixels(pt.x, pt.y, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, &clr);
		return clr;
	}
	...
	void PutPixel(const Point &pt, const Color &clr, uint32_t width = 1, uint32_t height = 1) {
		...
		glUniform4f(loc, clr.red / 255.0f, clr.green / 255.0f, clr.blue / 255.0f, clr.alpha / 255.0f);
